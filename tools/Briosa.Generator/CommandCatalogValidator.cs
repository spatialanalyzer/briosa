...
internal static partial class CommandCatalogValidator
{
    ...
    private const string ReleaseMembershipSchemaReference =
        "../../../schemas/v1/release-membership.schema.json";
    ...
    private static void ValidateReleaseMemberships(
        string catalogRoot,
    ...
    {
        RequireSorted(
            manifest.ReleaseMembershipFiles,
            manifestDisplayPath,
            "release_membership_files",
            errors);

        var releasesDirectory = Path.Combine(targetDirectory, "release-memberships");
        var actualReleaseFiles = Directory.Exists(releasesDirectory)
            ? Directory.EnumerateFiles(releasesDirectory, "*.json")
                .Select(path => NormalizePath(Path.GetRelativePath(targetDirectory, path)))
                .OrderBy(path => path, StringComparer.Ordinal)
                .ToArray()
            : [];
        var listedReleaseFiles = manifest.ReleaseMembershipFiles
            .OrderBy(path => path, StringComparer.Ordinal)
            .ToArray();
        if (!actualReleaseFiles.SequenceEqual(listedReleaseFiles, StringComparer.Ordinal))
        {
            errors.Add(
                $"{manifestDisplayPath}: release_membership_files must list every release-memberships/*.json file exactly once.");
        }

        var membershipIds = new HashSet<string>(StringComparer.Ordinal);
        foreach (var relativeReleasePath in manifest.ReleaseMembershipFiles)
        {
            var releasePath = Path.GetFullPath(Path.Combine(
                targetDirectory,
                relativeReleasePath.Replace('/', Path.DirectorySeparatorChar)));
            if (!IsWithin(releasePath, targetDirectory))
            {
                errors.Add(
                    $"{manifestDisplayPath}: release membership path '{relativeReleasePath}' escapes its target directory.");
                continue;
            }

            var releaseDisplayPath = DisplayPath(catalogRoot, releasePath);
            if (!File.Exists(releasePath))
            {
                errors.Add(
                    $"{manifestDisplayPath}: release membership file '{relativeReleasePath}' does not exist.");
                continue;
            }

            var membership = Deserialize<CommandCatalogReleaseMembership>(
                releasePath,
                releaseDisplayPath,
                errors);
            ...
            RequireEqual(
                membership.Schema,
                ReleaseMembershipSchemaReference,
                releaseDisplayPath,
                "$schema",
                errors);
            if (membership.SchemaVersion != 1)
            {
                errors.Add($"{releaseDisplayPath}: schema_version must be 1.");
            }
            ...
            RequireEqual(
                ...
                errors);
            RequireEqual(
                membership.SpatialAnalyzerTarget,
                manifest.SpatialAnalyzerTarget,
                releaseDisplayPath,
                "spatial_analyzer_target",
                errors);
            RequireEqual(
                membership.CatalogRevision,
                manifest.CatalogRevision,
                releaseDisplayPath,
                "catalog_revision",
                errors);
            if (!membershipIds.Add(membership.MembershipId))
            {
                errors.Add(
                    $"{releaseDisplayPath}: duplicate membership_id '{membership.MembershipId}'.");
            }

            var expectedPath = $"release-memberships/{membership.MembershipId}.json";
            RequireEqual(
                relativeReleasePath,
                expectedPath,
                releaseDisplayPath,
                "release membership file path",
                errors);
            RequireSorted(
                ...
                "operation_ids",
                errors);
            var duplicateOperationId = membership.OperationIds
                .GroupBy(operationId => operationId, StringComparer.Ordinal)
                .FirstOrDefault(group => group.Count() > 1);
            if (duplicateOperationId is not null)
            {
                errors.Add(
                    $"{releaseDisplayPath}: duplicate operation_id '{duplicateOperationId.Key}'.");
            }
            foreach (var operationId in membership.OperationIds)
            {
                ...
                {
                    errors.Add(
                        $"{releaseDisplayPath}: operation_id '{operationId}' is not present in the supported catalog.");
                }
            }
        }
    }
