...
internal static class CommandCatalogArtifactGenerator
{
    ...
    public static string GenerateServerOperations(
    ...
    public static string GenerateServiceRegistration(
        IReadOnlyList<(string GeneratedNamespace, string Service)> services)
    {
        var builder = new StringBuilder();
        builder.AppendLine("// <auto-generated />");
        builder.AppendLine("// Generated from the reviewed Briosa command catalog. Do not edit by hand.");
        ...
        builder.AppendLine();
        builder.AppendLine("namespace Briosa.Server.Generated;");
        builder.AppendLine();
        builder.AppendLine("[global::System.CodeDom.Compiler.GeneratedCode(\"Briosa.Generator\", \"1.0\")]");
        builder.AppendLine("internal static class CatalogServiceRegistration");
        builder.AppendLine("{");
        builder.AppendLine("    public static IEndpointRouteBuilder MapGeneratedCatalogServices(");
        builder.AppendLine("        this IEndpointRouteBuilder endpoints)");
        builder.AppendLine("    {");
        builder.AppendLine("        ArgumentNullException.ThrowIfNull(endpoints);");
        foreach (var service in services
                     .OrderBy(service => service.GeneratedNamespace, StringComparer.Ordinal)
                     .ThenBy(service => service.Service, StringComparer.Ordinal))
        {
            builder.Append("        endpoints.MapGrpcService<global::")
                .Append(service.GeneratedNamespace).Append('.')
                .Append(service.Service).AppendLine("Service>();");
        }
    ...
    public static string GenerateDocumentation(
        CommandCatalogManifest manifest,
        IReadOnlyList<CommandCatalogOperation> operations,
        IReadOnlyList<CommandCatalogReleaseMembership> releaseMemberships)
    {
        var builder = new StringBuilder();
        builder.AppendLine("<!-- <auto-generated /> -->");
        builder.AppendLine("<!-- Generated from the reviewed Briosa command catalog. Do not edit by hand. -->");
        ...
        builder.AppendLine();
        builder.AppendLine("For every listed operation the same generation pass emits its protobuf contract, request validation/mapping, immutable worker command, typed result mapping, gRPC service/registration, capability descriptor, and coverage record. Policy, supervision, error mapping, and auditing remain hand-written shared seams.");
        builder.AppendLine();
        ...
        builder.AppendLine();
        builder.AppendLine("## Release subset membership");
        builder.AppendLine();
        builder.AppendLine("Membership declarations identify additive reviewed delivery subsets, not the complete installed MP inventory and not cross-version compatibility claims.");
        foreach (var membership in releaseMemberships)
        {
            builder.AppendLine();
            builder.Append("- `").Append(membership.MembershipId).Append("` (`")
                .Append(membership.ReleaseLine).Append("`, `")
                .Append(membership.DeliveryWave).Append("`): ")
                .Append(membership.OperationIds.Count).AppendLine(" operation(s)");
        }
        ...
        IReadOnlyList<CommandCatalogReleaseMembership> releaseMemberships)
    {
        ...
        var document = new
        {
            ...
            release_memberships = releaseMemberships.Select(membership => new
            {
                membership.MembershipId,
                membership.ReleaseLine,
                membership.DeliveryWave,
                membership.CatalogId,
                membership.OperationIds
            }),
            operations = operations.Select(operation => new
            {
                operation.OperationId,
                release_memberships = releaseMemberships
                    .Where(membership => membership.OperationIds.Contains(
                        operation.OperationId,
                        StringComparer.Ordinal))
                ...
                generated = new
                {
                    ...
                    request_adapter = true,
                    immutable_worker_command = true,
                    result_adapter = true,
                    grpc_service = true,
                ...
                inputs = operation.Arguments.Where(IsInput).OrderBy(argument => argument.Ordinal)
                    .Select(argument => new
                    {
                        ...
                        argument_family_assignment = true,
        ...
        IReadOnlyList<CommandCatalogOperation> operations)
    {
        builder.AppendLine();
        builder.AppendLine("[global::System.CodeDom.Compiler.GeneratedCode(\"Briosa.Generator\", \"1.0\")]");
        builder.Append("internal sealed class ").Append(partition.Service)
            .AppendLine("Service(CatalogOperationExecutor operationExecutor) :");
        builder.Append("    TargetProtocol.").Append(partition.Service).Append('.')
            .Append(partition.Service).AppendLine("Base");
        builder.AppendLine("{");
        builder.AppendLine("    private readonly CatalogOperationExecutor _operationExecutor =");
        builder.AppendLine("        operationExecutor ?? throw new ArgumentNullException(nameof(operationExecutor));");

        foreach (var operation in operations)
        {
            var binding = operation.Protocol.Service + operation.Protocol.Rpc + "Binding";
            var descriptor = operation.Protocol.Rpc + "Operation";
            builder.AppendLine();
            builder.Append("    private static readonly CatalogOperationDescriptor ")
                .Append(descriptor).AppendLine(" =");
            builder.AppendLine("        TargetCatalogMetadata.Operations.Single(operation =>");
            builder.Append("            operation.OperationId == ").Append(binding)
                .AppendLine(".OperationId);");
            builder.AppendLine();
            builder.Append("    [OperationImplementation(").Append(binding)
                .AppendLine(".OperationId)]");
            builder.Append("    public override Task<TargetProtocol.")
                .Append(operation.Protocol.Result).Append("> ")
                .Append(operation.Protocol.Rpc).AppendLine("(");
            builder.Append("        TargetProtocol.").Append(operation.Protocol.Request)
                .AppendLine(" request,");
            builder.AppendLine("        ServerCallContext context) =>");
            builder.AppendLine("        _operationExecutor.ExecuteAsync(");
            builder.AppendLine("            request,");
            builder.AppendLine("            context,");
            builder.Append("            ").Append(descriptor).AppendLine(",");
            builder.Append("            ").Append(binding).AppendLine(".CreateCommand,");
            builder.Append("            ").Append(binding).AppendLine(".OutputContracts,");
            builder.Append("            ").Append(binding).AppendLine(".CreateResult);");
            builder.AppendLine();
            builder.Append("    internal Task<TargetProtocol.").Append(operation.Protocol.Result)
                .Append("> Execute").Append(operation.Protocol.Rpc).AppendLine("(");
            builder.Append("        TargetProtocol.").Append(operation.Protocol.Request)
                .AppendLine(" request,");
            builder.AppendLine("        CancellationToken cancellationToken,");
            builder.AppendLine("        DateTime? deadline = null,");
            builder.AppendLine("        Guid? correlationId = null,");
            builder.AppendLine("        string actorCategory = \"internal-unattributed\") =>");
            builder.AppendLine("        _operationExecutor.ExecuteAsync(");
            builder.AppendLine("            request,");
            builder.Append("            ").Append(descriptor).AppendLine(",");
            builder.Append("            ").Append(binding).AppendLine(".CreateCommand,");
            ...
            builder.AppendLine("            actorCategory);");
        }
