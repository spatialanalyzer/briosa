    ...
    Get-CimInstance Win32_Process |
        Where-Object {
            ...
            $_.ExecutablePath -like
                "*\SpatialAnalyzer 2024.1.0508.5\x64\Spatial Analyzer64.exe"
        })
if ($saProcesses.Count -ne 1) {
    throw "Exactly one SpatialAnalyzer 2024.1.0508.5 x64 process must already be running."
}
...
try {
    ...
    if (-not $diagnostics.ready_to_launch -or
        $diagnostics.spatial_analyzer_target -ne "2024.1.0508.5") {
