...
The protected workflow validates the packaged vertical slice against one separately licensed SpatialAnalyzer 2024.1.0508.5 x64 instance. It is not an ordinary pull-request check and must not run on a personal development workstation.
...
| Workflow access | Selected workflows: `spatialanalyzer/briosa/.github/workflows/licensed-sa-2024.yml@main` only |

A repository administrator must create an environment with these settings:

| Setting | Required value |
| --- | --- |
| Environment | `licensed-sa-2024-1-0508-5` |
...
Install SpatialAnalyzer 2024.1.0508.5 x64 and activate its separate license. Register the GitHub Actions runner at the organization level in `briosa-licensed-sa` with these labels:

```text
self-hosted,windows,x64,briosa-licensed,sa-2024-1-0508-5
```
...
3. Start exactly one SpatialAnalyzer 2024.1.0508.5 x64 instance and wait until it is ready. This clean sequence lets the first eligible instance acquire the observed SDK ports.
...
6. In GitHub Actions, select **Licensed SpatialAnalyzer 2024 validation**, choose `main`, enter the version established by each evidence record plus the two independent references, and dispatch the workflow. Do not copy the package target unless the retained evidence independently establishes that value.
7. Review the resolved source commit and hosted `prepare` job before approving the `licensed-sa-2024-1-0508-5` environment.
...
- 64-bit Windows;
- exactly one process from the expected SA 2024.1.0508.5 x64 installation;
