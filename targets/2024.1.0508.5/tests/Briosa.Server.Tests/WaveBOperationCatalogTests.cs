...
public sealed class WaveBOperationCatalogTests
{
    ...
    {
        Assert.Equal(533, WaveBOperationCatalog.Operations.Count);
        Assert.Equal(
            new Dictionary<string, int>(StringComparer.Ordinal)
            {
                ["briosa.CloudAndMeshOperations"] = 21,
                ["briosa.ConstructionOperations"] = 213,
                ["briosa.GdtOperations"] = 33,
                ["briosa.InstrumentOperations"] = 157,
                ["briosa.RelationshipOperations"] = 51,
    ...
    public void LaterGdtExtendedOptionsAreNotCallable()
    {
        Assert.DoesNotContain(Api.GdtOperations.Descriptor.Methods,
            method => method.Name is "SetGdtExtendedOptions" or "GetGdtExtendedOptions");
        Assert.DoesNotContain(WaveBOperationCatalog.Operations,
            operation => operation.Descriptor.Rpc is "SetGdtExtendedOptions" or "GetGdtExtendedOptions");
    }

    [Fact]
    public void InstrumentTypeNameAcceptsTheLegacyPmtArmAndRejectsLaterChoices()
    {
    ...
    {
        if (descriptor.FullName == "briosa.InstrumentTypeName")
        {
            return new Api.InstrumentTypeName { Value = "AICON DPA" };
        }
