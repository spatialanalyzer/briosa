# Briosa for SpatialAnalyzer 2024.1.0508.5
...
This is the initial implementation for SA 2024.1.0508.5. It is unreleased and has no licensed 2024 runtime validation. The [compatibility review](docs/development/sa2024-compatibility.md) explains the selected surface, exact-target differences, evidence provenance, and remaining release gates. The [instrument guide](docs/operations/legacy-instruments.md) documents the three additional instrument operations and their unexecuted opt-in scenarios.
...
./eng/Test-ClientScenarios.ps1 `
  -PackagePath artifacts/package-smoke/briosa-0.1.0-local-sa-2024.1.0508.5-win-x64.zip
```
...
The opt-in lifecycle scripts are retained for future licensed validation. No
2024 runtime result is claimed; see the [validation record](docs/development/sa2024-compatibility.md).
...
The [completed 2024 command review](https://briosa.dev/mp-command-catalog/2024.1.0508.5/review-notes) is the disposition reference. This target does not carry copies of the 2026 inventory, binding registry, value evidence, or runtime observations.
