...
internal static class GetVectorFromVectorGroupByNameOperation
{
    public static OperationDescriptor Descriptor { get; } = new(
        "vector_operations.get_vector_from_vector_group_by_name",
        "Get Vector From Vector Group By Name", "briosa.VectorOperations",
        "GetVectorFromVectorGroupByName", "/briosa.VectorOperations/GetVectorFromVectorGroupByName",
    ...
    public static WorkerMpCommand CreateCommand(Api.GetVectorFromVectorGroupByNameRequest request)
    {
    ...
    public static Api.GetVectorFromVectorGroupByNameResult CreateResult(SuccessfulOperationExecution completed)
    {
        var values = completed.Execution.OutputValues;
        return new()
        {
            BeginInWorking = VectorMapper.ToProtocol(values[0].RequireValue<WorkerVectorValue>()),
            EndInWorking = VectorMapper.ToProtocol(values[1].RequireValue<WorkerVectorValue>()),
            TotalDeltaInWorking = VectorMapper.ToProtocol(values[2].RequireValue<WorkerVectorValue>()),
            IjkUnitVectorInWorking = VectorMapper.ToProtocol(values[3].RequireValue<WorkerVectorValue>()),
