...
The Wave 2 fixture covers all three point-lifecycle operations across the same packaged boundary: successful construct mapping with the authoritative `vector3` type, missing-coordinate validation before worker execution, deny-overrides-allow for delete, and rename MP failure while `overwrite_if_exists` is omitted and the reviewed `false` default is applied. Its mutation failures preserve `unknown` replay safety and `DO_NOT_REPLAY` guidance. Reports contain no point names, coordinates, raw arguments, or returned values.
