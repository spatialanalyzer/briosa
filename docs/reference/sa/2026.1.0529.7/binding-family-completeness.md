...
Current support is narrower: only the calls required by implemented operations are product claims. `GetWorkingDirectory` uses `SetStep`, `ExecuteStep`, `GetMPStepResult`, and `GetStringArg("Directory", ...)`.
...
Historical registry coverage fields and generated reports belong to the retired catalog pipeline. See [the SDK binding reference guide](../../../development/sdk-binding-registry.md) and [ADR 0024](../../../architecture/0024-handwritten-mp-operation-vertical-slices.md).
