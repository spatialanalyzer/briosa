...
`Collection Item Name` is broader than `Collection Object Name`. Its exact `ItemType` list contains all 26 object choices plus `Alignment`, `Annotation`, `Calibration Appliance Node`, `Callout View`, `Chart`, `Dimension`, `Event`, `Feature Check`, `Picture`, `Relationship`, `SA Doc`, `SA Report`, `SA Report Template`, `Scale Bar`, `Table`, and `TCP Fixture`. `SetItemTypeArg` is absent from the committed exact-target interop API, so `item_type` remains `blocked_missing_interop`: Briosa publishes no `ItemType` and does not route it through `SetObjectTypeArg` or `SetStringArg`.
