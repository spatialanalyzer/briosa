...
The delimiter forms are based on the manually reviewed ObjectiveSA implementation for an earlier SA release, while the callable method names and CLR signatures come from the committed SA 2026.1.0529.7 interop manifest. Portable tests cover formatting, parsing, worker serialization, generated mappings, exact dispatch, empty lists, and failure behavior.

An authorized local conformance probe against SA 2026.1.0529.7 tested the exact array marshalling boundary on a single SDK-owning STA. After a successful `Get Defined Report Tags` execution, `GetStringRefListArg` threw `DISP_E_TYPEMISMATCH` (`0x80020005`) when initialized with a bare `object[]`; the same getter initialized with `VariantWrapper(Array.Empty<object>())` returned 13 strings. `SetStringRefListArg` showed the same type mismatch for a bare `object[]`, while a wrapped two-string array was accepted and `Append to String Ref List` completed with MP result code `2` and returned index `2`. No returned tag strings were recorded. This verifies the wrapper requirement on the tested string-list setter and getter; full cross-family protected-runner conformance remains pending.
