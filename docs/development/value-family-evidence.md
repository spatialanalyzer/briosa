...
The reviewed baseline produces 1,187 ObjectiveSA mappings, 719 exact-target setter samples, 421 active corroborated defaults, 314 reviewed candidates retaining required input, no pending entries, and 1,296 resolved inputs with no committed default candidate. Issue #80 contributes 25 resolved inputs across its four final file-operation candidates; their fail-closed contracts require explicit values and do not import uncommitted generated-sample values as defaults. Every retained decision records its candidate values, evidence state, reason codes, and issue #82 reference in the disposition shard. Re-running the script without `-DecisionProposalPath` preserves those decisions only when the recalculated evidence is byte-equivalent; drift fails closed. The generated queue at `generated/values/sa/2026.1.0529.7/default-review-queue.json` must remain empty.
