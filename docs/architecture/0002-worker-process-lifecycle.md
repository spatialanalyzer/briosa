...
- Protocol versions 1 and 2 established worker ready, heartbeat ping/pong, graceful stop, stopped acknowledgement, and connection readiness. Protocol version 3 adds the serialized execution messages defined by [ADR 0004](0004-mp-execution-pipeline.md). Protocol version 4 adds the exact catalog-reviewed SDK setter/getter binding name to each private argument descriptor, as defined by [ADR 0009](0009-catalog-derived-operation-artifacts.md). Protocol version 5 adds the exact-target identity and reference-list value records defined by issue [#55](https://github.com/spatialanalyzer/briosa/issues/55); delimiter-encoded SDK arrays remain private to the worker adapter.
