...
public sealed class DevelopmentGrpcReflectionTests
{
    ...
    public async Task DevelopmentHostReflectsEveryMappedServiceAndPreservesAdmissionPolicy()
    {
        ...
        var discovery = new CoreProtocol.DiscoveryService.DiscoveryServiceClient(host.Channel);
        var serverInfo = await discovery.GetServerInfoAsync(
            new CoreProtocol.GetServerInfoRequest()).ResponseAsync.ConfigureAwait(true);
        Assert.Equal("2026.1.0529.7", serverInfo.Version.SpatialAnalyzerTarget);
        Assert.False(serverInfo.ReadyForMp);
        Assert.Equal(
            CoreProtocol.RuntimeIdentityEvidenceSource.Unavailable,
        ...
        var capabilities = await discovery.ListCapabilitiesAsync(
            new CoreProtocol.ListCapabilitiesRequest()).ResponseAsync.ConfigureAwait(true);
        ...
        Assert.DoesNotContain(
            ...
            operation => operation.Effect == CoreProtocol.OperationEffect.Mutating);

        var fileOperations = new TargetProtocol.FileOperations.FileOperationsClient(host.Channel);
