...
public sealed class ExactTargetIdentityPolicyTests
{
    ...
    public void RuntimeMismatchWinsOverMatchingAttestation()
    {
        ...
        Assert.Equal(
            RuntimeIdentityEvidenceSource.RuntimeVerification,
            identity.ActivatedSdk.Source);
        Assert.Equal(RuntimeIdentityMatchState.Mismatch, identity.ActivatedSdk.MatchState);
    ...
    private static void AssertUnavailable(RuntimeIdentityEvidence evidence)
    {
        Assert.Null(evidence.Version);
        Assert.Equal(RuntimeIdentityEvidenceSource.Unavailable, evidence.Source);
        Assert.Equal(RuntimeIdentityMatchState.Unavailable, evidence.MatchState);
    }
}
