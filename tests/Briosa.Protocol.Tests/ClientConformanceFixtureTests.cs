...
public sealed class ClientConformanceFixtureTests
{
    ...
    private static readonly string[] ExpectedWave2ObjectLifecycleScenarios =
    [
        "copy-object-ready",
    ...
    [Fact]
    public void Wave2ObjectLifecycleFixtureCoversEveryOperation()
    {
        using var document = ReadFixture("wave2-object-lifecycle-scenarios.json");
        var root = document.RootElement;
        ...
        var scenarios = root.GetProperty("scenarios").EnumerateArray().ToArray();
        Assert.Equal(
            ExpectedWave2ObjectLifecycleScenarios,
            scenarios.Select(scenario => scenario.GetProperty("id").GetString())
                .Order(StringComparer.Ordinal));
        Assert.Equal(
            [
