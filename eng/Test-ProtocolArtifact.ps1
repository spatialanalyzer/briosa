...
try {
    ...
    $wave1FixturePath = Join-Path $bundleRoot "conformance\v1\wave1-read-only-scenarios.json"
    $wave1Fixtures = Get-Content -LiteralPath $wave1FixturePath -Raw | ConvertFrom-Json
    $expectedWave1Ids = @(
        "collection-count-mp-failure",
        "collection-count-policy-denied",
        "collection-count-ready",
        "collection-name-missing-index")
    $actualWave1Ids = @($wave1Fixtures.scenarios.id | Sort-Object)
    Assert-Condition `
        -Condition (-not (Compare-Object $expectedWave1Ids $actualWave1Ids)) `
        -Message "The Wave 1 read-only generated-client scenarios are incomplete."
    ...
    $actualWave2Ids = @($wave2Fixtures.scenarios.id | Sort-Object)
    Assert-Condition `
        -Condition (-not (Compare-Object $expectedWave2Ids $actualWave2Ids)) `
        -Message "The Wave 2 point-lifecycle generated-client scenarios are incomplete."

    $wave2CollectionFixturePath = Join-Path $bundleRoot "conformance\v1\wave2-collection-mutations-scenarios.json"
    $wave2CollectionFixtures = Get-Content -LiteralPath $wave2CollectionFixturePath -Raw | ConvertFrom-Json
    $expectedWave2CollectionIds = @(
    ...
    $actualWave2CollectionIds = @($wave2CollectionFixtures.scenarios.id | Sort-Object)
    Assert-Condition `
        -Condition (-not (Compare-Object $expectedWave2CollectionIds $actualWave2CollectionIds)) `
        -Message "The Wave 2 collection-mutation generated-client scenarios are incomplete."

    $wave2ObjectFixturePath = Join-Path $bundleRoot "conformance\v1\wave2-object-lifecycle-scenarios.json"
    $wave2ObjectFixtures = Get-Content -LiteralPath $wave2ObjectFixturePath -Raw | ConvertFrom-Json
    $expectedWave2ObjectIds = @(
        "copy-object-ready",
        "delete-objects-ready",
        "rename-object-ready")
    $actualWave2ObjectIds = @($wave2ObjectFixtures.scenarios.id | Sort-Object)
    Assert-Condition `
