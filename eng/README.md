...
- `Verify-CiWorkflow.ps1` checks the ordinary GitHub Actions trigger and permission policy.
...
- `Test-ClientScenarios.ps1` runs a standard generated gRPC client against the packaged server and fake worker. It covers success, unavailability, policy rejection, MP failure, getter failure, deadlines, cancellation, watchdog recovery, and unsupported target packages.
